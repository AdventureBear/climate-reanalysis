...
- Match each story to its most direct variable: heavy rain and flooding -> precip_rate or precipitable_water; severe-storm fuel -> cape; heat -> temp at level surface_2m; moisture and humidity -> rel_humidity or precipitable_water; the large-scale pattern -> height at 500mb. Don't reuse a map for a story that a more direct variable could show.
- To show where a boundary or front sat: in summer use dewpoint_2m (the moisture contrast stays sharp when the temperature contrast washes out); in winter use temp at surface_2m with contours ["temp"].
- To show what carried something — smoke, dust, moisture — use wind_speed at 700mb (or 500mb) with wind true, so the barbs show the flow direction.
