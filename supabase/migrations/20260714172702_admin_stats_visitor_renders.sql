-- Visitor log: include each visitor's individual renders (#14 follow-up).
-- Each visitor_log entry gains a 'renders' array (time, variable, level,
-- region, mode, time_scale) so the admin panel can list every render under
-- the visitor's hash. Same data that already feeds the aggregates; this is
-- display, not new collection.
-- Full body replacement (same pattern as 20260714170305); grants re-asserted.
